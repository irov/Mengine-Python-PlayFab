...
class PlayFabError(object):
    ...
    def GenerateErrorReport(self):
        report = []
        report.append("PlayFab Error")
        if self.HttpCode is not None:
            report.append("HttpCode: {}".format(self.HttpCode))
        if self.HttpStatus is not None:
            report.append("HttpStatus: {}".format(self.HttpStatus))
        if self.Error is not None:
            report.append("Error: {}".format(self.Error))
        if self.ErrorMessage is not None:
            report.append("ErrorMessage: {}".format(self.ErrorMessage))
        if self.ErrorDetails is not None:
            for key, details in self.ErrorDetails.items():
                for detail in details:
                    report.append(" {}: {}".format(key, detail))
